...
Lemma~\ref{lemma:augmented} guarantees us that every busy interval $A$ on processor $k$ is a section of the critical set $C$ with $C \sim A$.
...
\begin{algorithmic}
  \Function{$\fillop$}{$k, V$}
    ...
    with $A \sim V$ and $\hat \phi(A) \leq k - 1$}
      \State{let $V_l, V_r$ be the left and right valley for $C$ and interval $A$ (if $A$ is a section of $C$)}
...
\begin{lemma}\label{lemma:invariant}
  ...
  \begin{enumerate}
    ...
    \item
      Every busy interval $A \subseteq V$ on processor $k_V$ with $A \sim V$ is a section of $C$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  Assume for contradiction that $A$ is not a section of $C$.
...
\end{lemma}
\begin{proof}
  %The invariants in Lemma~\ref{lemma:invariant} imply that whenever we are inside the while-loop of the subroutine $\fillop$, the corresponding busy interval $A$ is indeed a section of the corresponding critical set $C$ with $C \sim A$ and that the left valley $V_r$ or the left valley $V_l$ is and there is sufficient $\res$ that one of them is closed.

  Since in the while-loop of $\fillop(k, V)$ the busy interval $A \subseteq V$ on $k_V$ always is a section of $C$ if $V \sim C$ (Invariant 3), the left valley $V_l$ and the right valley $V_r$ of the critical set $C$ and interval $A$ are properly defined.
...
\begin{proof}
  ...
  We know that on processor $k$ busy intervals are only extended, since in $\fillop(k, T)$ we only close valleys for busy intervals $A$ on $k$ which are a section of the corresponding critical set $C$.
