...
We show that the natural extension of Left-to-Right to multiple processors, which proceeds Top-Left-to-Bottom-Right when visually stacking the processors on top of each other, still guarantees a solution of costs $2 \OPT + P$, where $P$ is the total processing volume.
In particular, our simple greedy algorithm does not rely on LP-relaxations and the necessary rounding procedures of~\cite{antoniadis} and~\cite{skeletons}.
It also does not rely on the concept of multi-processor skeletons which is fundamental for the combinatorial $6$-approximation of~\cite{skeletons}.
More specifically, our simple generalization of Left-to-Right only relies on the original greedy policy: keep processors in their current state (busy or idle) as long as possible while guaranteeing feasibility.
For the case of a single processor, Left-to-Right ensures feasibility by scheduling jobs according to the fundamental earliest-deadline-first policy (EDF).
For the case of multiple processors, a simple flow calculation is required for calculating a feasible schedule, since EDF is not sufficient anymore.
Correspondingly, our generalization of Left-to-Right uses these flow calculations for ensuring feasibility.
\todo[inline]{The main technical contribution \ldots}
