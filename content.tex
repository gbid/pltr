...
The device has to complete a number of jobs, each with it's own interval during which it can be executed.
...
This fundamental problem in power management was first considered by~\cite{irani_left_to_right_soda_2003} in which they devised arguably the simplest algorithm one can think of for the case of a single processor.
...
Their dynamic programming algorithm is quite involved and builds upon the dynamic program devised by~\cite{baptiste_unit_jobs} for unit size jobs.
\todo[inline]{Describe DP approach?}

\paragraph{Multi-Processor Setting}
\todo[inline]{Cite that~\ref{baptiste_general_jobs} explicitly mentions multi-processor setting as open question?}
...
The first result for an arbitrary number of processors $m$ was given by~\ref{demaine}.
They solved the special case of jobs with unit processing volume by building on the dynamic program approach of~\cite{baptiste_unit_jobs}.
...
Interestingly, it took $13$ further years for the first non-trivial result on the general case of multiple processors and general processing volumes to be published.
