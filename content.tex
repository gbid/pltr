...
Every processor is either completely off or completely on in a discrete time slot $t$.
...
Each processor consumes $1$ unit of energy for every unit of time it is in the on-state and $0$ units of energy if it is in the off-state.
...
For a feasible solution to \emph{power-down scheduling with lower and upper bounds}, we require that in every time slot $t$, the number $b_t$ of busy processors, which we will denote with $\vol(t)$, lies within the lower and upper bounds, i.e.\ $l_t \leq v(t) \leq m_t$.
Note that this is related to the problem of \emph{deadline-scheduling-on-intervals} introduced by Antoniadis (TODO: ref).



\section{Algorithm}
\todo{Replace \emph{active, active intervals} with \emph{busy} everywere?}
...
\todo{Introduce notation for number of jobs scheduled at $t$, e.g. $\vol(t)$?}
...
\begin{lemma}\label{lemma:phi_prop}
  For every processor $k \in [m]$, every active interval $A$ on processor $k$ in $S_{\real}$ with $\crit(A) \geq 2$ we have $\hat \phi(A) > k - 1$.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  Since $A_r$ is an active interval with $\crit(A_r) \leq 1$ (i.e.\ not containing activations of processors above $1$ in $S_{\aug}$), we must then have $k = 1$.
...
\begin{proof}
  ...
  Since the realignment of $S_{\aug}$ only moves active slots between processors, but not between different time slots, we can easily restore $S_{\pltr}$ (up to permutations of the jobs scheduled on the active processors at the same time slot) by moving all active slots back down to the lower numbered processors.
...
\end{theorem}
\begin{proof}
  First observe that every active interval is created by a pair of calls to $\keepidle$ and $\keepactive$.
  ...
  Therefore, the total number of calls to $\keepidle$ and $\keepactive$ is bounded by $n + m$.
  ...
  We derive the upper bound of $n$ for the number of active intervals across all processors by constructing an injective mapping $f$ from the set of active intervals to the jobs $J$.
  ...
  We construct $f$ such that $f(A) = j$ for some active interval $A$ only if $d_j \in A$.
  ...
  \todo{Define plateaus: active intervals which are completely idle on higher processors}
