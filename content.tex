...
\todo{Energy consumption blabla: global energy consumption, battery powered devices}
\paragraph{Informal problem description}
One of the two fundamental power-management techniques are power-down mechanisms.
In the most basic setting, the computing device has two possible states, the active and the sleep state.
The active state allows the device to perform computations while consuming energy at a fixed rate.
In the sleep state, no energy is consumed but also no computations can be performed.
Transitioning from the sleep state to the active state incurs additonal energy consumption.
The goal is to perform all necessary work while putting the device into the sleep state when it is idle for long periods such that the overall consumed energy is minimal.
...
\todo{Talk about online/offline stuff?}
\todo{More formally define problem.}
\todo{Talk about parallel version already here}

\subsection{Previous work}
This fundamental problem in power management was first considered by~\cite{algorithmic_problems_in_power_management_irani} (TODO) in which they devised arguably the simplest algorithm one can think of for the case of a single processor.
...
Once there are no more released jobs to be worked on, the processor becomes idle and $\ltr$ greedily keeps the processor idle for as long as possible such that all remaining jobs can still be feasibly completed before their deadlines.
At this point, the processor becomes active again and $\ltr$ proceeds recursively until all jobs are completed.
...
Current consumer-grade processors are equipped with multiple cores and modern data centers consist of thousands of processors which in turn might contain millions of cores in total.
