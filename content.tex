...
\begin{abstract}
  ...
  Our algorithm has a running time of $\mathcal{O}(F n \log d)$, where $d$ is the difference between the largest deadline and the earliest release time, and $F$ the running time of a maximum flow calculation for checking feasibility in a network of $\mathcal{O}(n)$ nodes.
\end{abstract}
%\clearpage
...
\todo{Include $\epsilon$ in approximation factor here already?}
...
The initially described greedy algorithm Left-to-Right of~\cite{irani_left_to_right_soda_2003} is arguably the simplest algorithm one can think of which goes beyond mere feasibility for the case of a single processor.
...
Finally, in Section~\ref{section:running_time} we show that the simplicity of our algorithm also shows in its running time of $\mathcal{O}(F n \log d)$, where $d$ is the maximal deadline and $F$ the running time for finding a maximum flow in a network with $\mathcal{O}(n)$ nodes.
\todo{Use small $f$ instead of $F$ for our flow time?}
...
\todo{Include domains of $r_j, d_j, m$?}
...
\begin{proof}
  ...
  \todo{Think about what to do with this ``ending before $d$ restriction.}
...
\todo[inline]{Show how flow calculation for feasibility can be done faster.}
\begin{theorem}\label{theorem:running_time}
  Algorithm $\PLTR$ has a running time of $\mathcal{O}(n F_n \log d)$ where $F_n$ denotes the time needed for finding a maximum flow in a network with $\mathcal{O}(n)$ nodes.
...
\begin{proof}
  ...
  Having bounded the number of calls to $\keepidle$ and $\keepbusy$ by $\mathcal{O}(n)$, the final step is to bound the running time of these two subroutines by $\mathcal{O}(F_n \log d)$.
  We can modify the flow-network of Figure~\ref{fig:flow} to have only $\mathcal{O}(n)$ by partitioning $T$ into \textit{time intervals} instead of individual time slots.
  Since this is a standard problem as laid out in e.g.\ Section 5 of~\cite{brucker_scheduling_algorithms}, we only sketch the main points.
  This partition into time intervals is done by using the release times and deadlines as splitting points of the time horizon and scaling the capacity of the corresponding edges by the length of the time interval.
  For our problem we additionally have to split whenever an upper or lower bound $l_t, m_t$ changes.
  Since we have already bounded the number of such times by $n$ in the first part of the proof, there are only $\mathcal{O}(n)$ time intervals and hence also $\mathcal{O}(n)$ nodes in the flow network.
  ...
  This can be done in $O(n^2)$ for each time interval, see Section 5 of~\cite{brucker_scheduling_algorithms}.
  With the feasibility check running in time $\mathcal{O}(F_n)$, each call to $\keepidle$ and $\keepbusy$ can be completed in $\mathcal{O}(F_n \log d)$ using binary search on the remaining time horizon.
\end{proof}
