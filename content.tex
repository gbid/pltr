...
\begin{proof}
  ...
  \paragraph{Case 1:}
      ...
      \end{figure}
      Hence there must have been a busy interval $A'' \subseteq [\min A', \max A]$ on processor $k'_V + 1 > k_V$ before the close, which contradicts the choice of $k_V < k$.
...
\begin{proof}
  ...
  Using binary search this can be done in $O(F \log(d^*))$ with $F$ denoting the time required for finding the maximum flow.
  ...
  We call a busy interval $A'$ in $S_{\pltr}$ on processor $k' \in [m]$ a \emph{plateau on processor $k'$}, if all slots of $A'$ are idle for all processors above $k'$.
  ...
  Let $A'$ be the last plateau with $A' \subseteq A$ and let $k' \geq k$ be the processor for which this busy interval $A'$ is a plateau.
  By construction of $f$ and the choice of $A'$, there are at most $k' - k$ distinct jobs $j$ with $d_j \in [\min A', \max A]$ already mapped to by $f$.
  This is since at most $k' - k$ busy intervals on processors $k+1, \ldots, m$ intersect the interval $[\min A', \max A]$.
  ...
  Hence there are at least $k' - (k-1)$ distinct jobs $j \in J'$ with $d_j \in [\min A', \max A]$ and there must be at least one such job $j^*$ which is not mapped to by $f$ so far and which we therefore can assign to $A$.
\end{proof}
