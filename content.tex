...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{The Flow-Network for checking feasibility of a scheduling instance with lower and upper bounds $l_t$ and $m_t$ for the number of busy processors at $t \in T$.}\label{fig:flow}
\end{figure}
...
Intuitively, it states that for every activation $t$ of processor $k$ in the schedule $S_{\pltr}$ returned by $\PLTR$, there must be some set $Q$ of timeslots during which only the minimum amount of volume is scheduled by $\PLTR$. This in turn implies that an activation of processor $k$ is necessary in every feasible schedule.
The auxiliary Lemmas~\ref{lemma:cut} and~\ref{lemma:feasibility} provide a necessary and more importantly also sufficient condition for the feasibility of an instance of \emph{power-down scheduling with lower and upper bounds} based on the excess $\exc(Q)$ and the deficiency $\opdef(Q)$ of $Q \subseteq T$.
\todo{Also mention here, that this is a generalization of the feasibility characterization of~\cite{antoniadis}?}
...
\end{definition}
\todo{How to replace the term \emph{activation} (Antoniadis uses \emph{active} synonymous with \emph{on})? Candidates: \emph{start-of-work}.}
...
%\begin{lemma}\label{lemma:critical_sets_inclusion}
%  If $C \succeq C'$ for critical sets $C, C'$, then $C \supseteq C'$.
%\end{lemma}
%\begin{proof}
%  Let $t, t' \in T$ be the activations for which $C, C'$ are critical sets for and $k, k'$ the highest processors which are activated at $t, t'$ respectively.
%  If $C \succeq C'$, we have by definition that $k \geq k'$.
%\end{proof}
\begin{definition}
...
\end{definition}
\begin{definition}
  We extend the definitions of $\rank$ and $\crit$ to general time slots $t \in T$ and intervals $D \subseteq T$ as follows.
...
\todo{Sketch some valley $V$ of some critical set $C$.}
...
\end{definition}
\begin{definition}
  \todo{Think about using a different term than \emph{to span} here, e.g.\ \emph{to partially span} or \emph{is a section of $C$}.}
...
\begin{definition}
  ...
  %If $V$ is not defined, we define $J(V) \coloneqq \emptyset$.
\end{definition}
...
\begin{proof}
  ...
  These jobs must be scheduled in every $t \in V_l$ or in every $t \in V_r$ and hence be contained in $J(V_l)$ or $J(V_r)$ since they are scheduled in $C$ and the schedule is feasible.
...
More specifically, it will establish that $\hat \phi(A) > k - 1$ for every $\busy$ interval $A$ on processor $k$, i.e.\ that every feasible schedule requires $k$ busy processors at some point during $A$.
...
Instead of scheduling this job on the processors below $k$, we can schedule the job on processor $k$ in idle time slots during $V$.
...
This allows us to maintain the Invariants stated in Lemma~\ref{lemma:invariant} during the realignment process, which are analogous to Lemma~\ref{lemma:valley} and Lemma~\ref{lemma:augmented}.
...
\begin{proof}
  ...
  For the induction base, consider $S_{\aug}$, let $V$ be an arbitrary valley in $S_{\aug}$ with $\crit(V) \geq 2$ and let $C$ be the critical set with $C \sim V, c \coloneqq \crit(V)$.
...
\end{proof}
While with Lemma~\ref{lemma:phi_prop} we have our desired property for busy intervals of $\crit \geq 2$, we still have to handle busy intervals of $\crit \leq 1$.
To be precise, we have to handle idle intervals which are surrounded only by busy intervals of $\crit \leq 1$.
...
\begin{proof}
  ...
  Therefore, the constellation occurs exactly in the same way in $S_{\aug}$ and $S_{\pltr}$ on processor $1$, i.e.\ on processor $1$ in $S_{\aug}$ and $S_{\pltr}$, $A_l$, $A_r$ are busy intervals and $I$ is an idle interval, see Figure~\ref{fig:constellation}.
...
\begin{proof}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{graphics/running_time.jpg}
    \caption{}\label{fig:running_time}
  \end{figure}

  Let $C_t$ be the critical set over activation $t \coloneqq \min B$ of processor $l$.
