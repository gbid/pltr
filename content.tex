...
\todo[inline]{Energy consumption blabla: global energy consumption, battery powered devices}
Energy-efficiency has become a major concern in most areas of computing even beyond the apparent ecological reasons.
...
Algorithmic techniques for saving power in computing environments employ two fundamental mechanisms, namely the option to power-down idle devices and speed-scaling of processors.
In this paper, we study a fundamental scheduling setting with a power-down mechanism in a multi-device environment.

\paragraph{Informal problem description}
In this setting, a computing device or processor has two possible states, an active state and a sleep state.
The active state allows the device to perform computations while consuming energy at a fixed rate.
In the sleep state, the energy is consumed is negligible but no computations can be performed.
However, transitioning from the sleep state back to the active state consumes additonal energy.
The devices have to complete a number of tasks or jobs, each with its own processing volume and interval during which it has to be executed.
The goal is to complete all jobs during their respective execution intervals while minimizing the consumed energy.
Intuitively, one aims for long but few idle intervals, such that the energy required for transitioning between the states is low, while avoiding processors being in the active state but not performing work.
...
\todo[inline]{Formulate this better or leave it even out?}
The processor is defined to be in the sleep state for idle intervals which are sufficiently long such that it is worthwhile to transition to the sleep state.

\paragraph{Single Processor Setting}
\todo[inline]{Cite~\cite{irani_pruhs_pose_problem} for problem relevance as Antoniadis does?
Also mention like Demaine that the problem without objective functions is one of the fundamental scheduling settings}
...
They devised a dynamic program that runs in time $\mathcal{O}(n^7)$, where $n$ denotes the number of jobs to be scheduled.
...
\todo[inline]{Mention that in their conclusion, they pose the multi-processor setting with general jobs as a natural generalization of the problem, as this was also an ESA paper? Beware: they already mention~\cite{demaine} here.}
...
It took further thirteen years for the first non-trivial result on the general case of multiple processors and general processing volumes to be developed.
In their breakthrough paper,~\cite{antoniadis} developed the first constant-factor approximation for the problem.
...
Their algorithm obtains an integer solution by building the convex hull of the corresponding fractional solution.
Since this integer solution might not schedule all jobs, they develop an additional algorithm \textit{EXT-ALG} based on maximum flow calculations which iteratively searches for a time slot at which an additional active processor is able to schedule one processing unit of some unfinished job.
By incorporating additional constraints for the number of processors required during all possible time intervals into this Linear Program and by modifying the rounding procedure,~\cite{skeletons} improve this approximation factor to $2$.
This modified rounding procedure builds on their concept of a \textit{skeleton}.
Very roughly, a skeleton is a stripped-down schedule which still guarantees a number of active processors during specific intervals and which provides a lower bound for the cost of an optimal schedule.
Building on this concept of skeletons, they also develop a combinatorial $6$-approximation for the problem.
Both Linear Programs have $\mathcal{O}(d^2)$ constraints and $\mathcal{O}(d^2)$, where $d$ denotes the maximum deadline and hence the number of time slots.
All three algorithms also critically rely on the $EXT-ALG$ to obtain a feasible solution, which requires time...

\cite{skeleton} also gives a combinatorial $6$-approximation for the problem 
\todo[inline]{Describe their techniques and emphasise that they are complex.}
\todo[inline]{Mention combinatorial $6$-approximation in~\cite{skeletons}.}
\todo[inline]{Cite~\cite{antoniadis} why this problem seems to be hard: \textit{The
...
from just one to all available machines.}}
It is a major open problem whether the problem in the general case is NP hard.
\todo[inline]{Get handle on running time of previous multi-processor algorithms:}
\begin{enumerate}
  \item Combinatorial 6-approximation by~\cite{skeletons}:
    \begin{enumerate}
      \item Computing lower bounds on required processors for every possible interval of $r_i, d_i$:
        $O(F n^2 \log(m))$
      \item Computing multi-processor skeleton based on these lower bounds, i.e. $m$ single processor skeletons for $n' = O(n^2)$ virtual unit-size jobs in $O(n' \log n')$:
        $O(n^2 \log^2(n))$
      \item Extension phase via Flow calculation: normally $O(P F)$, with standard technique big time slots $|W| \in O(n \log(d^*))$: active intervals are extended maximally, such that the deficiency is reduced by the same amount: $O(m |W|^3)$ iterations, so in total:
        $O(F m |W|^3) = O(F m n^3 \log^3(d^*))$ with $F$ refering to flow caluclation with $O(|W|) = O(n \log(d^*))$ vertices $v_t$ (according to~\cite{antoniadis}, the ``size of the network after doing the above modification is $O(n|W|)$'').
      \item Tripling phase: setting $m'_{t} \coloneqq \min\{m, 3 m_t\}$: $O(1)$?
      \item In total dominated by extension phase requiring time $O(F m n^3 \log^3(d^*))$ with $F$ refering to flow calculation in network of size $O(n |W|) = O(n^2 \log(d^*))$.
    \end{enumerate}
  \item Original 3-approximation using LPs by~\cite{antoniadis}:
    \begin{enumerate}
      \item
    \end{enumerate}
\end{enumerate}
...
The previously mentioned greedy algorithm Left-to-Right is arguably the simplest algorithm one can think of for the case of a single processor.
...
Our simple greedy algorithm Parallel Left-to-Right ($\PLTR$) does not rely on Linear Programming and the necessary rounding procedures of~\cite{antoniadis} and~\cite{skeletons}.
It also does not rely on the concept of multi-processor skeletons at the heart of the combinatorial $6$-approximation algorithm of~\cite{skeletons}.
Indeed, $\PLTR$ only relies on the original greedy policy of Left-to-Right: keep processors in their current state (busy or idle) for as long as feasibly possible.
...
While our devised algorithm is very simple, the analysis is highly non-trivial.
The structure exhibited by the Parallel $\LTR$ schedule is surprisingly rich.
...
In Lemma~\ref{lemma:critical} we show that whenever $\PLTR$ requires an additional processor to become busy at some timeslot $t$, there must exist a critical set of timeslots containing $t$.
This in turn gives a lower bound for the number of busy processors required by any solution.
Note that critical sets of timeslots are the kind of structure which~\cite{antoniadis} describe when they state that approximation algorithms are hard to develop because ``$\ldots$ structural properties of an optimal schedule can be locally extracted in a single machine environment in contrast to
the multi-machine case. As an example, we know that a single machine will be active for at least
one time-point within the interval between the release time and the deadline of every job, but
the number of active machines at such a time-point in the multiprocessor setting could range
from just one to all available machines''.
\todo[inline]{Delete this quote?}

Devising an approximation guarantee from this structure is however much more involved than the approximation proof of the single-processor $\LTR$ algorithm, because one has to deal with sets of timeslots and not just intervals.
Our main contribution in terms of techniques is a complex procedure which (for the sake of the analyis only) carefully realigns the jobs scheduled in between critical sets of timeslots such that it is sufficient to consider intervals, as described in Section~\ref{section:realignment}.
...
\todo[inline]{Compare the running time to some of the following: number of LP constraints in~\cite{antoniadis} and~\cite{skeletons}, combinatorial 6-approximation by~\cite{skeletons} (is the latter even really polynomial?) }
\todo[inline]{Include faster Flow Calculation here and talk about pseudopolynomial, polynomial running time here?}
\todo[inline]{Mention that it is surprising that the running time is somehow independent on $m$ since it only appears in the capacities of the flow calculation.}
...
\end{proof}
From now on whenever we consider an optimal schedule, we will silently assume based on Lemma~\ref{lemma:stair_property_opt} that this optimal schedule fulfills the stair property.
...
\end{algorithmic}
\end{algorithm}
\todo[inline]{Is it possible to formulate and format $\keepbusy$ and  $\keepidle$ even more concise?}
As we will show in Lemma~\ref{lemma:flow_feasibility}, the check for feasibility in subroutines $\keepidle$ and $\keepbusy$ can be performed by calculating a maximum $\alpha$-$\omega$ flow in the flow network given in Figure~\ref{fig:flow}.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{The Flow-Network for checking feasibility of an instance of \emph{power-down scheduling with lower and upper bounds} $l_t$ and $m_t$ for the number of busy processors at $t \in T$.
...
\end{proof}
\todo[inline]
{Move this Theorem~\ref{theorem:feasibility} to the back together with Lemma~\ref{lemma:flow_feasibility}.
...
\begin{definition}
  A time slot $t \in T$ is called \emph{activation of processor $k \in [m]$} if $t = \min A$ for some busy interval $A$ on processor $k$.
  A time slot $t \in T$ is just called \emph{activation} if it is an activation of processor $k$ for some $k \in [m]$.
\end{definition}
\todo[inline]{How to replace the term \emph{activation} (\cite{antoniadis} uses \emph{active} synonymous with \emph{on})? Candidates: \emph{start-of-work, start of busy interval, becoming busy}.}
\begin{lemma}\label{lemma:critical}
  Let $Q \subseteq T$ be a set of time slots and $t \in T$ an activation of processor $k \in [m]$.
  We call $Q$ a \emph{tight set for activation $t$ of processor $k$} if $t \in Q$ and
...
Lemma~\ref{lemma:valley} in turn implies that if the density of $A$ is too small to guarantee that $S_{\opt}$ has to use processor $k$ during $A$, i.e.\ if $\hat \phi(A) \leq k - 1$, then $V_l$ or $V_r$ is defined and there is some $j$ scheduled in every slot of $V_l$ or $V_r$.
...
\begin{proof}
  ...
  In conclusion, we have $\costs(S_{\real}) = \idle(S_{\real}) + \busy(S_{\real}) \leq 2 \off(S_{\opt}) + \on(S_{\opt}) + 2 P \leq 2 \OPT + P$.
\end{proof}
