...
\begin{proof}
  ...
  \begin{figure}[H]
    ...
    %\caption{}\label{fig:engagements}
  \end{figure}
  \todo[inline]{Decide if I want to draw this in tikZ}
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}
      % Schedule after the close label
      \node[anchor=east, align=center] (schedule_after) at (0, 5) {Schedule\\ after the close};

      % k' processor
      \node[anchor=west] (k_prime) at (1, 5) {$k'$};

      % Idle time slot (single horizontal line)
      \draw[line width=0.6pt] (2.5, 5) -- (3.5, 5);

      % Busy time slot with t label (without right border)
      \draw[line width=0.6pt, fill=gray!30] (3.5, 4.5) rectangle (4.5, 5.5);
      \node at (4, 5) {$t$};
      \draw[line width=0.6pt] (3.5, 4.5) -- (3.5, 5.5);
      \draw[line width=0.6pt] (3.5, 5.5) -- (4.5, 5.5);
      \draw[line width=0.6pt] (3.5, 4.5) -- (4.5, 4.5);

      % Remove the right border of the square
      \fill[gray!30] (4.485, 4.51) rectangle (4.511, 5.49);
    \end{tikzpicture}
  \end{figure}
