...
One of the two fundamental power-management techniques are power-down mechanisms.
In the most basic setting, the computing device has two possible states, an active state and a sleep state.
The active state allows the device to perform computations while consuming energy at a fixed rate.
In the sleep state, no energy is consumed but no computations can be performed.
Transitioning from the sleep state to the active state consumes additonal energy.
The device has to complete a number of jobs, each with its own interval during which it has to be executed.
The goal is to complete all jobs during their respective execution intervals while putting the device into the sleep state when it is idle for long periods such that the overall consumed energy is minimal.
Intuitively, one aims for long but few idle intervals, during which it is worthwhile to put the device into the sleep state.
\todo[inline]{Talk about parallel version already here}

\paragraph{Previous work}
This fundamental problem in power management was first considered by~\cite{irani_left_to_right_soda_2003} in which they devised arguably the simplest algorithm one can think of for the special case of a single processor.
...
(TODO) The processor is defined to be in the sleep state for idle intervals which are sufficiently long such that it is worthwhile to transition to the sleep state.
...
The problem for a single processor was solved optimally by~\cite{baptiste_general_jobs} in time $\mathcal{O}(n^5)$.
Their dynamic programming algorithm is quite involved and builds upon the dynamic program devised by~\cite{baptiste_unit_jobs} for jobs with unit processing volume.
\todo[inline]{Describe why DP approach and why it is very complex}
\todo[inline]{Mention and/or describe approximation algorithms by~\cite{antoniadis} and~\cite{skeletons} or is this bad because it mentions a small running time?}

\paragraph{Multi-Processor Setting}
\todo[inline]{Cite that~\cite{baptiste_general_jobs} explicitly mentions multi-processor setting as open question?}
\todo[inline]{Move the motivation for multi-processor setting to first paragraph}
Small battery powered state-of-the-art devices such as smartphones are already equipped with four to eight processors while modern data centers consist of thousands of servers which might in total contain millions of cores.
The multi-processor case is therefore becoming more relevant than in recent years.
The first result for an arbitrary number of processors $m$ was given by~\cite{demaine}.
They solved the special case of jobs with unit processing volume by building on the dynamic programming approach of~\cite{baptiste_unit_jobs}.
Their algorithm gives an optimal schedule and runs in time $\mathcal{O}(n^7p^5)$.
Obtaining good solutions for general job weights is difficult because every job can be worked on by at most a single processor at the same time.
Note that this is no additional restriction for the former special case of unit processing volumes since time is discrete in our problem setting.
Interestingly, it took further thirteen years for the first non-trivial result on the general case of multiple processors and general processing volumes to be developed.
...
This factor was further improved to $2$ in~\cite{skeletons}.
