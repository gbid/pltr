...
This in turn implies that an activation of processor $k$ is necessary in every feasible schedule.
...
\end{lemma}
\begin{proof}
  We show the existance by induction over the processors $m, \ldots 1$.
  We construct a critical set $C_m$ for processor $m$ taking the union over all tight sets over activations of processor $m$.
  ...
  By our induction hypothesis, $Q_k \supseteq C_{k'}$ for all $k' > k$.
  By construction, every activation $t$ of processor $k$ is contained in $Q_k$.
...
A nonempty interval $V$ is a \emph{valley of $C$} if $V$ is exactly the valley between $C_{a}$ and $C_{a+1}$ for some $a < l$, i.e. $V = [\max C_a + 1, \min C_{a+1} - 1]$.
