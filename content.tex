...
Instead of scheduling this job on the processors below $k$, we can schedule the job on processor $k$ in idle time slots during $V$ instead of the lower processors, merging the busy interval $A$ with at least one neighbouring busy interval on processor $k$.
...
\begin{proof}
  ...
  \paragraph{Case 1:}
      ...
      Also, since $\close(k, \_)$ only moves the busy slot of the highest busy processor below $k$, we know that the stair property \todo{define stair property} holds within $V$ on processors $1, \ldots, k$.
