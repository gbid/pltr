...
In the sleep state, the energy consumed is negligible but no computations can be performed.
...
Since this integer solution might not schedule all jobs, they develop an additional \textit{extension} algorithm \textit{EXT-ALG} based on maximum flow calculations. EXT-ALG iteratively searches for a time slot at which an additional active processor allows for an additional unit of processing volume to be scheduled.
...
Very roughly, a skeleton is a stripped-down schedule which still guarantees a number of active processors during specific intervals and which provides a lower bound for the costs of an optimal feasible schedule.
Building on this concept of skeletons, they also develop a combinatorial $6$-approximation for the problem.
\todo[inline]{Describe this combinatorial algorithm based on my Latex-notes.}

Both Linear Programs of~\cite{antoniadis} and~\cite{skeletons}, respectively, have $\mathcal{O}(d^2)$ constraints and $\mathcal{O}(d^2)$ variables, where $d$ denotes the maximum deadline or equivalently the number of time slots.
...
More specifically, the number of constraints and variables of the Linear Programs reduces to $\mathcal{O}(n^2 \log d)$.
...
The previously mentioned greedy algorithm Left-to-Right of~\cite{irani_left_to_right_soda_2003} is arguably the simplest algorithm one can think of for the case of a single processor that goes beyond mere feasibility.
$\LTR$ just keeps the processor in its current state (busy or idle) for as long as feasibly possible.
In this paper we naturally extend $\LTR$ to multiple processors and show that this generalization still guarantees a solution of costs at most $2 \OPT + P$, where $P$ is the total processing volume.
...
It also does not require the EXT-ALG, which all previous algorithms rely on to make an infeasible schedule feasible.
...
While our devised algorithm is very simple and the structure exhibited by the resulting schedules is surprisingly rich, the analysis is highly non-trivial.
...
Our main contribution in terms of techniques is a complex procedure which (for the sake of the analyis only) carefully realigns the jobs scheduled in between critical sets of timeslots such that it is sufficient to consider intervals, see Section~\ref{section:realignment} for details.
...
\todo[inline]{Faster Flow Calculation: mention size of network, that we don't rely on techniques of~\cite{antoniadis} for reducing number of time slots and hence do not incur additional $\epsilon$.}
