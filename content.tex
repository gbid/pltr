...
\newcommand{\algrule}[1][.2pt]{\par\vspace{0.5\baselineskip}\hrule height #1\par\vspace{0.5\baselineskip}}


\DeclareMathOperator{\prev}{prev}
\DeclareMathOperator{\on}{\textsf{on}}
\DeclareMathOperator{\off}{\textsf{off}}
\DeclareMathOperator{\idle}{\textsf{idle}}
\DeclareMathOperator{\busy}{\textsf{busy}}
\DeclareMathOperator{\costs}{\textsf{costs}}
...
\DeclareMathOperator{\fv}{\textsf{fv}}
\DeclareMathOperator{\uv}{\textsf{uv}}
\DeclareMathOperator{\pv}{\textsf{pv}}
\DeclareMathOperator{\vol}{\textsf{vol}}
\DeclareMathOperator{\opdef}{\textsf{def}}
\DeclareMathOperator{\exc}{\textsf{exc}}
\DeclareMathOperator{\keepidle}{\texttt{keepidle}}
\DeclareMathOperator{\keepbusy}{\texttt{keepbusy}}
\DeclareMathOperator{\rank}{\textsf{rank}}
\DeclareMathOperator{\crit}{\textsf{crit}}
\DeclareMathOperator{\fillop}{\texttt{fill}}
\DeclareMathOperator{\close}{\texttt{close}}
...
\title{Greedy Minimum-Energy Scheduling\thanks{This work was supported by the Research Training Network of the Deutsche Forschungsgemeinschaft (DFG) (378803395: ConVeY).}}
\author{Gunther Bidlingmaier\inst{1}}
\authorrunning{G.\ Bidlingmaier}
\institute{Department of Computer Science, Technical University of Munich, Germany
\\\email{g.bidlingmaier@tum.de}
}
...
Each idle processor can be turned off to save energy, while turning it on requires a fixed amount of energy.
For the special case of a single processor, the greedy Left-to-Right algorithm \cite{irani_left_to_right_soda_2003} guarantees an approximation factor of $2$.
...
\keywords{Scheduling  \and Greedy Algorithms \and Approximation Algorithms}
\end{abstract}
%\clearpage
...
%\subsubsection{Informal problem description}
...
Once there are no more released jobs to be worked on, the processor becomes idle and $\ltr$ keeps the processor idle for as long as possible such that all remaining jobs can still be feasibly completed.
At this point, the processor becomes busy again and $\ltr$ proceeds recursively until all jobs are completed.
For a single processor,~\cite{baptiste_unit_jobs} develop an optimal dynamic program for unit jobs.
~\cite{baptiste_general_jobs} generalize this to general job weights with a running time of $\mathcal{O}(n^5)$,
while~\cite{demaine} generalize it to multiple processors but again only unit jobs, increasing the complexity to $\mathcal{O}(n^7m^5)$.

Obtaining good solutions for the general case of multiple processors and general job weights is difficult because of the additional constraint that every job can be worked on by at most a single processor at the same time.
%Note that this is not an additional restriction for the former special cases since time is discrete in our problem setting.
It is a major open problem whether the general multi-processor setting is NP-hard.
It took further thirteen years for the first non-trivial result on this general setting to be be developed.
In their breakthrough paper,~\cite{antoniadis} develop the first constant-factor approximation for the problem.
Their algorithm guarantees an approximation factor of $3 + \epsilon$ and relies on relaxing an Integer Programming formulation of the problem.
For making the rounded LP-solution feasible, they develop an additional extension algorithm \textit{EXT-ALG}.
This approximation factor is improved to $2 + \epsilon$ in~\cite{skeletons} by incorporating into the Linear Program additional constraints for the number of processors required during every possible time interval.
They also develop a combinatorial $6$-approximation for the problem.
As presented in the papers, all three algorithms run in pseudo-polynomial time.
...
The running time of the EXT-ALG used by all three approximation algorithms is reduced to $\mathcal{O}(F m n^3 \log^3 d)$, where $F$ refers to a maximum flow calculation in a network with $\mathcal{O}(n \log d)$ nodes.
...
Here, we also show that our greedy policy leads to a much faster algorithm than the previous ones, namely to a running time $\mathcal{O}(n f \log d)$, where $d$ is the maximal deadline and $f$ is the running time for checking feasibility by finding a maximum flow in a network with $\mathcal{O}(n)$ nodes.
...
In Graham's notation~\cite{graham}, this setting can be denoted with $m \mid r_j; \overline{d_j}; \mathrm{pmtn} \mid E$.
\subsubsection{Busy and idle intervals}
...
An interval $I \subseteq T$ is a (full) \emph{busy interval} for processor $k \in [m]$ if $I$ is inclusion maximal on the condition that processor $k$ is busy in every $t \in I$.
Correspondingly, an interval $I \subseteq T$ is a \emph{partial busy interval} for processor $k$ if $I$ is not inclusion maximal on the condition that processor $k$ is busy in very $t \in I$.
...
Furthermore, for fixed $k$, let $\mathcal{N}, \mathcal{F}, \mathcal{B}, \mathcal{I}$ be the set of on, off, busy, and idle intervals of $S^k$. 
We partition the costs of processor $k$ into the costs $\on(S^k)$ for residing in the on-state and the costs $\off(S^k)$ for transitioning between the off-state and the on-state, hence $\costs(S^k) = \on(S^k) + \off(S^k) = \sum_{N \in \mathcal{N}} q + |N|$.
...
\subsubsection{Lower and upper bounds for the number of busy processors}
...
For a feasible solution to \emph{deadline-scheduling-with-processor-bounds}, we require that in every time slot $t$, the number of busy processors, which we denote with $\vol(t)$, lies within the lower and upper bounds, i.e.\ $l_t \leq \vol(t) \leq m_t$.
...
\subsubsection{Properties of an optimal schedule}
...
\algrule
  \Function{$\keepidle$}{$k, t$}
    \State{find maximal $t' > t$ s.t.\
    $\exists$ feasible schedule with $m_{t''} = k-1$ for all $t'' \in [t, t')$}
    \State{$m_{t''} \gets k - 1$ for all $t'' \in [t, t')$}
    \State{\Return{$t'$}}
  \EndFunction{}
  \Function{$\keepbusy$}{$k, t$}
    \State{find maximal $t' > t$ s.t.\
    $\exists$ feasible schedule with $l_{t''} = \max\{k, l_{t''}\}$ for all $t'' \in [t, t')$}
...
\begin{figure}
  ...
  There are nodes $u_j$, $v_t$ with the corresponding edges for every job $j \in J$ and for every time slot $t \in T$, respectively.
  }\label{fig:flow}
\end{figure}
...
\begin{definition}
  A time slot $t \in T$ is called an \emph{engagement of processor $k$} if $t = \min B$ for some busy interval $B$ on processor $k$.
	We say processor $k$ is \emph{engaged} at time $t$ if $t$ is an engagement of processor $k$.
  A time slot $t \in T$ is just called an \emph{engagement} if it is an engagement of processor $k$ for some $k \in [m]$.
\end{definition}
...
\end{lemma}
\begin{proof}
  Suppose for contradiction that there is some engagement $t \in T$ of processor $k \in [m]$ and no such $Q$ exists for $t$, i.e.\ every $Q \subseteq T$ containing $t$ violates at least one of the three conditions in the Lemma.
  ...
  At the step in $\PLTR$ considered by us, i.e.\ when $\keepidle$ returned $t$ on processor $k$, we hence have $m_{t'} \geq k - 1 > \vol(t')$.
  ...
  In summary, if for $t$ no $Q$ exists as characterized in the lemma, the engagement of processor $k$ at $t$ could not have been the result of $\keepidle$ on processor $k$.
\end{proof}
...
\begin{definition}
A nonempty interval $V \subseteq T$ is a \emph{valley} if $V$ is inclusion maximal on the condition that $C \sim V$ for some fixed critical set $C$.
...
By the choice of $C$ as a critical set (property 1), a valley of $C$ is indeed a valley.
...
\begin{figure}
  \centering
  \includegraphics[width=1\textwidth]{graphics/left_right_valleys.drawio.pdf}
...
\end{figure}
\begin{proof}
  Refer to Figure~\ref{fig:left_right_valleys} for a visual sketch of the lemma.
  By the choice of $C$ as a critical set with $c \coloneqq \crit(C)$, we have $\vol(C \cap D) \geq (c-1) \cdot |C \cap D|$.
...
In the definition of the realignment, we will call this process of filling the idle slots during $V$ on processor $k$ the \emph{closing of valley $V$ on processor $k$}.
...
We also allow every busy slot to be used twice for the realignment (see variable $\res_V$ in Algorithm~\ref{alg:real}) by introducing further auxiliary busy slots, since for a section $D$ of the critical set $C$, both the right and the left valley might be closed on processor $k$ in the worst case.
This allows us to maintain the invariants stated in Lemma~\ref{lemma:invariant} during the realignment process, which correspond to the initial properties of Lemmas~\ref{lemma:valley} and~\ref{lemma:augmented} for $S_{\aug}$.
\begin{algorithm}
  \caption{Realignment of $S_{\aug}$ for analysis only}\label{alg:real}
\begin{algorithmic}
  \State{$\res_V \gets 2 |J(V)|$ for every valley $V$}
  \For{$k \gets m$ \textrm{to} $1$}
    \State{\Call{$\fillop$}{$k, T$}}
    \State{$\res_V \gets \res_V - 1$  for every $V$
    s.t.\ some $V'$ with $V' \cap V \neq \emptyset$ was closed on proc. $k$}
  \EndFor{}
\algrule
  \Function{$\fillop$}{$k, V$}
    \If{$\crit(V) \leq 1$}
    ...
    with $B \sim V$ and $\hat \phi(B) \leq k - 1$}
      \State{let $V_l, V_r$ be the left, right valley for $C$ and $B$ (given $B$ is a section of $C$)}
			\If{$V_l$ exists and $\res_{V_l} > 0$}
        \State{\Call{$\close$}{$k, V_l$}}
			\ElsIf{$V_r$ exists and $\res_{V_r} > 0$}
  ...
  \Function{$\close$}{$k, V$}
    \For{every $t \in V$ which is idle on processor $k$}
      ...
      \Else{}
        \State{move busy slot $t$ of highest processor $\leq k-1$ to processor $k$}
      \EndIf{}
    \EndFor{}
  \EndFunction{}
\end{algorithmic}
...
\begin{restatable}{lemma}{lemmainvariant}\label{lemma:invariant}
  ...
  \begin{enumerate}
    \item
			If $\phi(C \cap D) \leq k_V - \delta$ for some $\delta \in \mathbb{N}$, some section $D \subseteq V$ of $C$, then the left valley $V_l$ or the right valley $V_r$ of $C, D$ exists and $\res_{V_l} + \res_{V_r} \geq 2 \delta$.
    \item
...
%\begin{restatable}{lemma}{lemmaengagements}\label{lemma:engagements}
...
\begin{restatable}{lemma}{lemmaconstellation}\label{lemma:constellation}
...
\subsection{Approximation Guarantee and Running Time}
...
\end{theorem}
\begin{proof}
  We begin by bounding $\costs(S_{\real})$ as in the lemma.
  ...
  By construction of $S_{\aug}$ and the definition of $\res_V$ and $\close$, we introduce at most as many auxiliary busy slots at every slot $t \in T$ as there are jobs scheduled at $t$ in $S_{\pltr}$.
  For $S_{\aug}$, an auxiliary busy slot is only added for $t$ with $\crit(t) \geq 2$ and hence $\vol(t) \geq 1$.
  Furthermore, initially $\res_V = 2 |J(V)|$ for every valley $V$ and $\res_V$ is decremented if some $V'$ intersecting $V$ is closed during $\fillop(k, T)$.
...
\begin{proof}
  ...
  With $k_V \leq c$ we get $\phi(C \cap D) \leq c - \delta$ and hence by Lemma~\ref{lemma:valley}, we have that the left valley $V_l$ or the right valley $V_r$ of $C$ and $D$ exists and $|J(V_l)| + |J(V_r)| \geq \delta$.
	With the initial definition of the supply $\res_V$ of a valley, we get the desired lower bound of $\res_{V_l} + \res_{V_r} \geq 2 \delta$.
  ...
  \subsubsection{Case 1:}
      ...
      \begin{figure}[H]
        \centering
        \includegraphics[scale=0.4]{graphics/invariant_case1.drawio.pdf}
      ...
      In conclusion, we have $k'_V \leq k_V - 1$, which allows us to prove Invariants 1 and 2.
			If $\phi(C \cap D) \leq k'_{V} - \delta$ for some $\delta \in \mathbb{N}$ and some section $D$ of $C$, then $\phi(C \cap D) \leq k_V - (\delta + 1)$ and hence by induction hypothesis the left valley $V_l$ or the right valley $V_r$ for $C, D$ exists and $\res_{V_l} + \res_{V_r} \leq 2 (\delta + 1)$ both before and after the close.
  ...
  \subsubsection{Case 2:}\label{case:2}
      ...
      For Invariant 1, again let $\phi(C \cap D) \leq k'_V - \delta$ and hence $\phi(C \cap D) \leq k_V - \delta$ for some $\delta \in \mathbb{N}$ and some section $D$ of $C$.
			Our induction hypothesis implies that the left valley $V_l$ or the right valley $V_r$ of $C, D$ exists and that both before and after the close we have $\res_{V_l} + \res_{V_r} \geq 2 \delta$.
  ...
  \subsubsection{Case 3:}
      ...
      By the choice of $V$ and $t$ we must have $t \succ V$.
  ...
  \subsubsection{Case 4:}
			The call to $\fillop(k, T)$ returns and $\res_{V'}$ is decreased by 1 for every valley $V'$ such that some valley intersecting $V'$ has been closed during $\fillop(k, T)$.
      ...
      We consider two subcases.
			If during $\fillop(k, T)$, no valley $V'$ intersecting $V$ was closed on $k$, then $\res_{V}$ does not change and Invariant 1 follows from the induction hypothesis and $k'_V \leq k_V$.

			If on the other hand some valley $V'$ intersecting $V$ was closed on $k$ during $\fillop(k, T)$, then $\res_{V}$ is decreased by $1$ to $\res'_V \coloneqq \res_{V} - 1$.
      ...
      Invariant 2 now follows by our induction hypothesis.
			If $\phi(C \cap D) \leq k'_V - \delta$ then $\phi(C \cap D) \leq k^0_V - (\delta + 1)$ and hence by our induction hypothesis the left valley $V_l$ or the right valley $V_r$ of $C, D$ exists and before the close we have $\res_{V_l} + \res_{V_r} \geq 2 (\delta + 1)$.
			Since $\res_V$ is decreased for every valley $V$ by at most $1$, we have after the close that $\res'_{V_l} + \res'_{V_r} \geq 2 \delta$.
...
%  \begin{figure}[H]
%    \centering
%		\includegraphics[scale=0.4]{graphics/engagements.drawio.pdf}
...
\begin{proof}
  ...
  Since no jobs are scheduled at $\max I$, we know that $\crit(\max I) \leq 1$ and $J(V) = \emptyset$ for all valleys $V$ containing the slot $\max I$, and hence also $\res_V = 0$ at all times during the realignment.
