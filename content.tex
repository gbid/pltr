...
%\thanks{A comprehensive version of this paper, including all proofs, is available on arXiv:.}
}
...
When idle, each processor can be turned off to save energy, while turning it on requires a fixed amount of energy.
...
Our algorithm has a running time of $\mathcal{O}(n f \log d)$, where $d$ is the difference between the last deadline and the earliest release time, and $f$ is the running time of a maximum flow calculation in a network of $\mathcal{O}(n)$ nodes.
...
Their algorithm guarantees an approximation factor of $3 + \epsilon$ by relaxing an Integer Programming formulation of the problem.
...
\begin{proof}
  ...
  With $\vol(t) \leq c - 1$ for every $t \in C \cap D$, we know that $\crit(C \cap D) \leq c$ and therefore $C \setminus D$ is still a critical set for processor $c$ but has higher density than $C$, contradicting the choice of $C$.
...
A comprehensive version of this paper, including all proofs, is available on arXiv: \href{https://arxiv.org/abs/2307.00949}{https://arxiv.org/abs/2307.00949}.
