...
\begin{definition}
  A time slot $t \in T$ is called \emph{activation of processor $k \in [m]$} if $t = \min B$ for some busy interval $B$ on processor $k$.
...
\begin{proof}
  ...
  \paragraph{Case 1:}
      ...
      \end{figure}
      \todo[inline]{Rename busy intervals in figure: $A: B, A': B', B: B''$.}
...
\end{lemma}
\begin{proof}
  By Lemma~\ref{lemma:activations} and $\crit(B_r) \leq 1$, we know that $\min B_r$ is an activation of processor $1$ in $S_{\aug}$.
  ...
  Since $B_r$ is a busy interval with $\crit(B_r) \leq 1$ (i.e.\ not containing activations of processors above $1$ in $S_{\aug}$), we must then have $k = 1$.
