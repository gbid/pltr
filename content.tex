...
\end{algorithmic}
\end{algorithm}
As we will show in Lemma~\ref{lemma:flow_feasibility}, the feasibility check in subroutines $\keepidle$ and $\keepbusy$ can be performed by calculating a maximum $s$-$t$ flow in the flow network given in Figure~\ref{fig:flow}.
...
\begin{figure}[H]
  \begin{tikzpicture}[node distance={25mm}, thick, main/.style = {draw, circle}]
    ...
    \node[main] (4) [above right of=3] {$w$}; 
    \node[main] (5) [below right of=4] {$t$}; 
    \draw[->] (1) -- node[midway, above] {$p_j$} (2);
    \draw[->] (2) -- node[midway, above] {$1$ if $t \in E_j$} node[midway, below] {$0$ otherwise} (3);
    \draw[->] (3) -- node[midway, above left] {$m_t - l_t$} (4);
    \draw[->] (3) -- node[midway, above] {$l_t$} (5);
    \draw[->] (4) -- node[midway, above right] {$P - \sum_{t \in T} l_t$} (5);
