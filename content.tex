...
\title{A Greedy Algorithm for Minimum-Energy Scheduling or \\ Greedy Minimum-Energy Scheduling}
...
\begin{abstract}
  ...
  We generalize this simple greedy policy to the case of multiple parallel processors and show that the energy costs are still bounded by $2 \OPT + P$, where $\OPT$ is the energy consumed by an optimal solution and $P < \OPT$ is the total processing volume.
  Our algorithm has a running time of $\mathcal{O}(n f \log d)$, where $d$ is the difference between the latest deadline and the earliest release time, and $f$ the running time of a maximum flow calculation in a network of $\mathcal{O}(n)$ nodes.
\end{abstract}
\clearpage
