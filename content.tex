...
\end{definition}
\begin{definition}
  We define the \emph{space $\spac_S(j, Q)$ of job $j \in J$ in a set $Q \subseteq [0, d^*]$ for schedule $S$} as the number of additional time slots, which $j$ can be scheduled in $Q$, i.e.\
  \begin{align}
...
\begin{definition}
  ...
  \begin{align}
    ...
    \end{cases}
  \end{align}
  We also extend the definitions to intervals $D \subseteq [0, d^*]$.
  \begin{align}
...
\end{definition}
\begin{definition}
  Let $C$ be a critical set. A nonempty interval $V \subseteq [0, d^*]$ is a \emph{valley of $\rank(C)$} if $C \sim V$ and $V$ is inclusion maximal.
...
\begin{lemma}\label{lemma:augmented}
  In $S_{\aug}$, in every $t \in [0, d^*]$ with $crit(t) \geq 2$ processors $1, \ldots, crit(t)$ are active.
\end{lemma}
\begin{proof}
...
\begin{lemma}
  ...
  \begin{itemize}
    \item
      processor $k_V$ is not fully filled yet, i.e.\ $\fillop(k_V, [0, d^*])$ has not yet returned,
    \item
  ...
  \begin{enumerate}
    \item
      If $\phi(C \cap D) \leq k_V - \delta$ for some $\delta \in \mathbb{N}$ and some interval $D \subseteq [0, d^*]$ spanning $C$, then the left or right valley $V_l, V_r$ of $C, D$ exists and $\res(V_l) + \res(V_r) \geq 2 \delta$.
    \item
...
\begin{proof}
  ...
  For the next step, we have to discern four cases.
  \begin{enumerate}
    \item
    ...
    \item 
      The call to $\fillop(k, [0, d^*])$ returns and $\res(V')$ is decreased by 1 for every $V'$ such that some valley intersecting $V'$ has been closed during $\fillop(k, [0, d^*])$.
  \end{enumerate}
  \begin{case}
    foo
  \end{case}
