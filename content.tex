...
\begin{abstract}
  ...
  Our algorithm has a running time of $O(n \cdot \log(d^*) \cdot F)$, where $d^*$ is the largest deadline and $F$ the time required by the maximum-flow calculation for checking the feasibility of an instance.
\end{abstract}
...
\begin{theorem}
  Algorithm $\pltr$ has a running time of $O(n \cdot \log(d^*) \cdot F)$, where $F$ is the time needed for the flow calculation in Figure~\ref{fig:flow}.
\end{theorem}
\begin{proof}
