...
\end{algorithmic}
\end{algorithm}
\todo[inline]{Is is possible to formulate and format $\keepbusy$ and  $\keepidle$ even more concise?}
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{The Flow-Network for checking feasibility of an instance of \emph{power-down scheduling with lower and upper bounds} with lower and upper bounds $l_t$ and $m_t$ for the number of busy processors at $t \in T$.
...
\begin{proof}
  ...
  During $\fillop(k, T)$ every $t \in T$ is closed at most once (or rather at most a single $V'$ containing $t$ is closed).
