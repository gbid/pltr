...
\begin{figure}[H]
  ...
  There is a node $u_j$ and $v_t$ with the corresponding edges for every job $j \in J$ and for every time slot $t \in T$, respectively.
  }\label{fig:flow}
\end{figure}
...
\begin{proof}
  ...
  During $\fillop(k, T)$ every $t \in T$ is closed at most once (or rather one $V'$ containing $t$ is closed).
