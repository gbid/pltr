...
Finally, in Section~\ref{section:running_time} we show that the simplicity of our algorithm also leads to a faster running time than the previous algorithms, namely $\mathcal{O}(n f \log d)$, where $d$ is the maximal deadline and $f$ is the running time for checking feasibility by finding a maximum flow in a network with $\mathcal{O}(n)$ nodes.
